...
async fn list_versions(
    app_name: String,
    app_version: String,
    cache: &State<Arc<Mutex<Box<dyn Cache + Send>>>>,
    versions_api: &State<Box<dyn VersionsApi + Send + Sync>>,
) -> StatusVersion {
    ...
    match m_cache.get(&key) {
        ...
        None => match versions_api.list(&app_name, &app_version) {
            Ok(versions) => {
                ...
                if versions.len() == 0 {
                    return StatusVersion::HttpNotFound(format!(
                        "app {} {} not found",
...
async fn list_changes(
    ...
    db_version: String,
    cache: &State<Arc<Mutex<Box<dyn Cache + Send>>>>,
    changes_api: &State<Box<dyn ChangesApi + Send + Sync>>,
) -> StatusChanges {
    ...
    match m_cache.get(&key) {
        ...
        None => match changes_api.get(&app_name, &db_version) {
            ...
            Err(_e) => StatusChanges::HttpNotFound(format!(
                "db version {} of app {}",
                db_version, app_name
            )),
        },
...
impl Api {
    pub fn init(&self) -> Rocket<Build> {
        let cache: Arc<Mutex<Box<dyn Cache + Send>>> =
            Arc::new(Mutex::new(Box::new(new_hash_cache())));
        self.clear_cache_periodically(&cache);
        let versions_api: Box<dyn VersionsApi + Send + Sync> =
            Box::new(new_sqlite_versions_api(&self.db_path));
        let changes: Box<dyn ChangesApi + Send + Sync> =
            Box::new(new_file_changes_api(&self.apps_path));
        let config = Config::figment().merge(("port", self.port));
...
mod tests {
    ...
    fn get_changes_insert_in_cache_when_not_found() {
        ...
        mock_cache
            .expect_insert()
            .with(
                eq("changes:name:v1".to_string()),
                eq("CREATE TABLE antani (id int)".to_string()),
            )
            .times(1)
            .returning(|_, _| ());
        let arc_cache: Arc<Mutex<Box<dyn Cache + Send>>> =
    ...
    fn clear_cache() {
        ...
        let mut mock_cache = MockCache::default();
        mock_cache.expect_clear().times(1).returning(|| ());
        let arc_cache: Arc<Mutex<Box<dyn Cache + Send>>> =
            Arc::new(Mutex::new(Box::new(mock_cache)));
